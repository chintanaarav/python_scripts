...
class PurchaseOrderCreation:
    def create_purchase_order( order_data,file_name,file_id, error_folder_id):
         ...
         try:
           ...
           created_id = models.execute_kw(db, uid, password, 'purchase.order', 'create',  [order_data])  
           #confirming purchase order
           models.execute_kw(db, uid, password, 'purchase.order', 'button_confirm', [created_id])   
           #models.execute_kw(db, uid, password, 'purchase.order', 'write', [[created_id], {'date_approve': order_data['date_order']}]) #writing backdated PO date             
    ...
    def update_purchase_order( order_data,file_name,file_id, error_folder_id,models,uid,db,password):
      ...
      try:
          ...
          purchase_order = models.execute_kw(db, uid, password, 'purchase.order', 'search_read', 
          ...
          result = models.execute_kw(db, uid, password, 'purchase.order', 'write', [[purchase_order_id], order_data])
